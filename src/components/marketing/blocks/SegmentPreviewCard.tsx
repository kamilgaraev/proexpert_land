...
const SegmentPreviewCard = ({ segment, linkTo }: SegmentPreviewCardProps) => {
  return (
    <article className="h-full rounded-[2.25rem] border border-steel-200 bg-white p-8 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-[0_30px_80px_-50px_rgba(15,23,42,0.45)] lg:p-9">
      <div className="text-xs font-semibold uppercase tracking-[0.24em] text-construction-700">
        {segment.title}
      </div>
      <h3 className="mt-5 max-w-xl text-[2rem] font-bold leading-tight text-steel-950">
        {segment.audience}
      </h3>
      <p className="mt-5 text-base leading-8 text-steel-600">{segment.challenge}</p>
      ...
      {linkTo ? (
        <Link
          to={linkTo}
          className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-steel-950 transition hover:text-construction-700"
        >
          {segment.cta}
