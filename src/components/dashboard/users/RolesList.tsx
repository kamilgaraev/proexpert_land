...
const RolesList: React.FC<RolesListProps> = ({ roles, loading }) => {
  const formatPermissions = (permissionsFormatted: Record<string, Array<{slug: string, name: string, granted: boolean}>> | undefined) => {
    if (!permissionsFormatted) return 'Нет разрешений';
    
    try {
      const granted = Object.values(permissionsFormatted)
        .flat()
        .filter(p => p && p.granted)
        .map(p => p.name);
      ...
      console.error('Ошибка форматирования разрешений:', error);
      return 'Ошибка загрузки разрешений';
    }
  };
    ...
    return (
      <div className="p-6">
        <div className="text-center py-12">
          <h3 className="text-lg font-medium text-gray-900">Ошибка загрузки ролей</h3>
              ...
              <div key={role.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    />
                    <h3 className="text-sm font-medium text-gray-900">{role.name || 'Без названия'}</h3>
                  </div>
                ...
                <p className="text-sm text-gray-600 mb-3">{role.description || 'Описание отсутствует'}</p>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500">
                    Пользователей: {role.users_count || 0}
                  </span>
