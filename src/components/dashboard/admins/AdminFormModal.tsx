...
import { adminPanelUserService, AdminUser, AdminFormData } from '@utils/api';

interface AdminFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onFormSubmit: () => void; // Общий callback после успешной операции
  adminToEdit?: AdminUser | null; // Администратор для редактирования
}
...
const AdminFormModal: React.FC<AdminFormModalProps> = ({ isOpen, onClose, onFormSubmit, adminToEdit }) => {
    ...
    if (isOpen) {
      if (isEditing && adminToEdit) {
        setFormData({
          ...
          role_slug: adminToEdit.role as 'web_admin' | 'accountant',
      ...
      // Сбрасываем состояние при закрытии, если не внутри useEffect с isOpen
      setFormData(initialFormState);
      setError(null);
    }
  }, [isOpen, isEditing, adminToEdit]); // Добавлены isEditing и adminToEdit в зависимости
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (formData.password) { // Отправляем пароль только если он был введен
      ...
      if (isEditing && adminToEdit) {
        ...
        // При создании пароль обязателен, поэтому dataToSend будет содержать его
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <select
                        ...
                        className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2"
                      >
                        <option value="web_admin">Веб-администратор</option>
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <input
                        ...
                        minLength={isEditing && !formData.password ? undefined : 8} // Мин. длина, если пароль вводится
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <input
                        ...
                        required={!isEditing && !!formData.password} // Обязательно, если вводится пароль при создании или редактировании
