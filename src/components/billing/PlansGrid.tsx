...
const PlansGrid = () => {
  ...
  if (loading) return (
     <div className="flex items-center justify-center py-20">
        <div className="text-center">
           ...
           <p className="text-muted-foreground">Загрузка информации о тарифах...</p>
        </div>
     </div>
  );
  ...
  return (
    <div className="space-y-10">
        ...
        <CardContent className="p-8">
          ...
          {subscription && (subscription.status === 'active' || subscription.status === 'canceled_active') ? (
            <div className="space-y-8">
              ...
              <div className="flex flex-col md:flex-row justify-between gap-6 pb-8 border-b border-border">
                <div className="flex gap-5">
                   ...
                   <div>
                      <div className="flex items-center gap-3 mb-1">
                         <h3 className="text-2xl font-bold text-foreground">{subscription.plan?.name || 'Тариф'}</h3>
                         <Badge variant={subscription.status === 'canceled_active' ? "secondary" : "default"} className={subscription.status === 'canceled_active' ? "bg-yellow-100 text-yellow-700 hover:bg-yellow-100" : "bg-emerald-100 text-emerald-700 hover:bg-emerald-100"}>
                            {subscription.status === 'canceled_active' ? 'Отменён (активен)' : 'Активен'}
                         </Badge>
                      </div>
                      <p className="text-muted-foreground mb-3">{subscription.plan?.description}</p>
                ...
                <div className="flex flex-col items-end gap-4">
                   <div className="text-right">
                      <p className="text-sm text-muted-foreground font-medium mb-1">Действует до</p>
                      <p className="text-lg font-bold text-foreground">{formatDate(subscription.ends_at)}</p>
                   </div>
                   
                   <div className="flex items-center gap-3 bg-secondary/50 px-4 py-2 rounded-xl border border-border">
                      <span className="text-sm font-medium text-foreground">Автопродление</span>
                      <Switch
              ...
              {subscription.plan?.features && (
                 <div className="bg-secondary/30 rounded-2xl p-6 border border-border/50">
                    <h3 className="text-sm font-bold text-foreground uppercase tracking-wider mb-4 opacity-80">Включенные опции</h3>
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-8">
                       {Object.entries(subscription.plan.features as Record<string, string[]>).map(([category, items]) => (
                          <div key={category}>
                             <div className="text-xs font-bold text-primary mb-2 uppercase">{category}</div>
            ...
            <div className="text-center py-12">
               ...
               <h3 className="text-xl font-bold text-foreground mb-2">Нет активной подписки</h3>
               <p className="text-muted-foreground max-w-md mx-auto">Выберите подходящий тариф ниже, чтобы получить доступ ко всем возможностям платформы.</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      {/* Plans Grid */}
      <section>
         <h2 className="text-2xl font-bold text-foreground mb-6">Доступные тарифы</h2>
           ...
           {Array.isArray(plans) && plans.map(plan => {
              ...
              return (
                 <Card 
                    key={plan.id}
                    className={cn(
                      "relative flex flex-col transition-all duration-300 group border-2",
                      isActive 
                          ? "border-primary shadow-xl shadow-orange-100 ring-4 ring-orange-50" 
                          : "border-border hover:border-orange-300 hover:shadow-lg"
                    )}
                 >
                    {isActive && (
                       <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-primary text-white px-4 py-1 rounded-full text-xs font-bold shadow-lg uppercase tracking-wider">
                          Текущий план
                       </div>
                    )}
                    ...
                    <CardHeader>
                       ...
                       <div className="flex items-baseline gap-1">
                          ...
                          <span className="text-muted-foreground font-medium">/ месяц</span>
                       </div>
                    ...
                    <CardFooter>
                      <Button
                         ...
                         className={cn(
                           "w-full font-bold",
                           isActive ? "bg-secondary text-muted-foreground hover:bg-secondary" : "bg-primary hover:bg-orange-600"
                         )}
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex gap-3">
               ...
               {changePlanModal.previewData?.can_proceed && (
                  <Button
                     ...
                     className="flex-1 py-3 h-auto bg-orange-600 hover:bg-orange-700"
                  >
