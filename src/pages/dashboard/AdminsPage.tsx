...
import { adminPanelUserService, AdminUser } from '@utils/api'; // AdminUsersApiResponse больше не нужен здесь напрямую
import AdminFormModal from '@components/dashboard/admins/AdminFormModal'; // Обновленный импорт
import ConfirmDeleteModal from '@components/shared/ConfirmDeleteModal'; // Импорт модального окна подтверждения
...
const AdminsPage = () => {
  ...
  const [editingAdmin, setEditingAdmin] = useState<AdminUser | null>(null); // Для редактирования
  const [isConfirmDeleteModalOpen, setIsConfirmDeleteModalOpen] = useState(false); // Правильное определение
  const [deletingAdmin, setDeletingAdmin] = useState<AdminUser | null>(null); // Для удаления
  ...
  const handleOpenDeleteConfirmModal = (admin: AdminUser) => {
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8">
      <div className="sm:flex sm:items-center">
        ...
        <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
          <button
            type="button"
            onClick={handleOpenCreateModal} // Используем новый обработчик
