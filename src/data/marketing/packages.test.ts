...
describe('коммерческий каталог пакетов', () => {
  it('точно отражает серверный каталог из рабочего входа и семи контуров', () => {
    ...
    expect(commercialPackages.map(({ slug, name, price }) => [slug, name, price])).toEqual([
      ...
      ['pto-handover', 'ПТО и сдача', 9_900],
  ...
  it('зеркалит серверный состав бесплатной базы и модулей каждого пакета', () => {
    ...
    expect(commercialPackages.map(({ slug, moduleSlugs }) => [slug, moduleSlugs])).toEqual([
      ['working-entry', ['site-requests', 'file-management', 'ai-assistant', 'data-export', 'schedule-management', 'budget-estimates', 'rate-management', 'ai-estimates']],
  ...
  it('фиксирует серверные описания, акценты и бизнес-результаты всех пакетов', () => {
    expect(commercialPackages.map((item) => ({
      ...
      { slug: 'working-entry', description: 'Объекты, заявки, графики и сметы в одном платном минимуме.', highlights: ['Заявки с объекта', 'Графики', 'Сметы и нормы', 'Помощник'], outcomes: ['Единый порядок работы по объектам', 'Управляемые сроки', 'Единая сметно-нормативная база'] },
  ...
  it('фиксирует цену и экономию полного комплекта', () => {
    ...
    expect(separateTotal).toBe(98_200);
    expect(fullSuiteOffer.price).toBe(79_900);
    expect(fullSuiteOffer.savings).toBe(18_300);
    expect(fullSuiteOffer.savingsPercent).toBe(18.64);
  ...
  it('сам добавляет рабочий вход к контуру и рекомендует комплект от 64 000 ₽', () => {
    ...
    expect(getCommercialSelection(['working-entry', 'supply-warehouse', 'finance-contracts', 'pto-handover']).recommendFullSuite).toBe(true);
