  ...
  it("сохраняет предметный язык и честные ограничения ручных SEO-кластеров", () => {
    const contracts = {
      "foreman-software": {
        ...
        limitation: /полнота зависит[^}]+регулярно[^}]+фиксир/iu,
      },
      "construction-crm": {
        terms: ["клиент", "договор", "объект", "этап", "обязательств"],
        limitation: /бухгалтерский учёт остаётся в профильной системе/iu,
      },
      "construction-erp": {
        terms: ["ресурс", "бюджет", "объект", "управленческ", "учёт"],
        limitation:
          /полнота отчёта зависит[^}]+справочников[^}]+обмена данными/iu,
      },
      "material-accounting": {
      ...
      "construction-documents": {
        ...
        limitation:
          /юридическая значимость зависит[^}]+регламента компании[^}]+электронной подписи \(эп\)/iu,
      },
      "construction-budget-control": {
        terms: ["бюджет", "лимит", "обязательств", "факт", "отклонен"],
        limitation:
          /точность зависит[^}]+все заявки, договоры и оплаты[^}]+связаны со статьями/iu,
      },
    } as const;
