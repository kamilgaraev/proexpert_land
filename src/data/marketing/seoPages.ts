...
export const marketingSeoLandingPages: Record<string, MarketingSeoLandingPage> = {
  ...
  'pir-project-documentation': createOperationalSeoPage({
    ...
    metrics: [
      { label: 'Версии', value: 'Под контролем', detail: 'Команда видит актуальные проектные материалы и историю изменений.' },
      { label: 'Замечания', value: 'Со статусами', detail: 'Замечания получают ответственных и не теряются в переписке.' },
      { label: 'Комплекты', value: 'Готовятся быстрее', detail: 'Выпуск ПД/РД строится на понятном составе и готовности разделов.' },
    ],
  ...
  'construction-safety': createOperationalSeoPage({
    ...
    metrics: [
      { label: 'Скорость реакции', value: 'В тот же день', detail: 'Нарушение или инцидент сразу попадает в рабочий маршрут.' },
      { label: 'Прозрачность устранения', value: 'Выше', detail: 'Руководитель видит открытые нарушения и предписания по объектам.' },
      { label: 'Связь с объектом', value: 'Единая', detail: 'Охрана труда не живет отдельно от площадки и подрядчиков.' },
    ],
  ...
  'construction-quality-control': createOperationalSeoPage({
    ...
    metrics: [
      { label: 'Замечания', value: 'Не теряются', detail: 'Каждый дефект имеет статус и ответственного.' },
      { label: 'Повторная проверка', value: 'Управляемая', detail: 'Команда видит, что нужно проверить повторно.' },
      { label: 'Готовность приемки', value: 'Прозрачная', detail: 'Качество связано с зонами и передачей результата.' },
    ],
  ...
  'handover-acceptance': createOperationalSeoPage({
    ...
    metrics: [
      { label: 'Punch-list', value: 'В одном месте', detail: 'Замечания заказчика не распадаются на отдельные таблицы.' },
      { label: 'Готовность зон', value: 'Прозрачная', detail: 'Команда видит статус передачи по зонам и объектам.' },
      { label: 'Риск срыва сдачи', value: 'Ниже', detail: 'Блокирующие замечания и документы видны заранее.' },
    ],
  ...
  'machinery-and-labor': createOperationalSeoPage({
    ...
    metrics: [
      { label: 'Простои', value: 'Видны быстрее', detail: 'События по технике фиксируются как рабочий факт.' },
      { label: 'Выработка', value: 'Проверяемая', detail: 'Наряды и объемы связаны с объектом и периодом.' },
      { label: 'Начисления', value: 'На базе факта', detail: 'Финансовый блок получает меньше ручной пересборки.' },
    ],
  ...
  'change-control': createOperationalSeoPage({
    ...
    metrics: [
      { label: 'RFI', value: 'Под контролем', detail: 'Запрос имеет срок ответа и статус.' },
      { label: 'Влияние', value: 'Видно заранее', detail: 'Изменение связывается со сроками, сметой и платежами.' },
      { label: 'История', value: 'Сохраняется', detail: 'Основания, доказательства и решения не теряются.' },
    ],
