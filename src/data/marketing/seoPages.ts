...
export const marketingSeoLandingPages: Record<string, MarketingSeoLandingPage> = {
    ...
    "mobile-app": {
      ...
      processComparison: createProcessComparisonFromSource({
        ...
        afterState: [
          "Задачи, фото, замечания и подтверждения сохраняются в карточках объекта.",
          "Офис и площадка проверяют один статус и историю его изменения.",
          "Полевые записи становятся основанием для следующего действия офиса.",
        ],
        metrics: [
          {
    ...
    "ai-estimates": {
      ...
      processComparison: createProcessComparisonFromSource({
        ...
        afterLabel: "После запуска",
        afterState: [
          "Система формирует предварительную структуру и передаёт её на проверку.",
  ...
  'construction-safety': createOperationalSeoPage({
    ...
    processComparison: createProcessComparison({ eyebrow: 'Безопасность работ', title: 'Нарушение проходит от фиксации до проверки устранения', description: 'HSE раскрывается как охрана труда, промышленная и экологическая безопасность; события связаны с объектом и ответственными.', metrics: [{ value: 'Инструктаж', label: 'Допуск', description: 'Запись связана с участником и видом работ.' }, { value: 'Предписание', label: 'Устранение', description: 'Нарушение имеет исполнителя и повторную проверку.' }], note: 'МОСТ хранит рабочие записи, но обязательные журналы и решения ответственных специалистов ведутся по правилам компании.' }),
