...
export const marketingSeoLandingPages: Record<string, MarketingSeoLandingPage> = {
    "foreman-software": {
      ...
      description:
        "Назначайте задачи на смену, добавляйте фотографии и ведите журнал работ. Программа для прораба сохраняет задания и результаты по участкам, чтобы офис видел ход строительства.",
      supportingQueries: [
      ...
      workflow: {
        ...
        description: "Утром — задания по участкам. В течение смены — фотографии и записи. В конце — проверка выполненных работ.",
        stages: [
          {
            label: "План смены",
          ...
          {
            label: "Проверка",
            description: "Руководитель проверяет результат и отмечает, что нужно исправить.",
          },
        ],
      },
      relatedLinks: [
    ...
    "material-accounting": {
      ...
      description:
        "Ведите учёт материалов по складам и объектам: оформляйте приход, перемещение и списание на работы. МОСТ сохраняет историю движения и рассчитывает остаток по внесённым операциям.",
      supportingQueries: [
    ...
    "mobile-app": {
      ...
      workflow: {
        ...
        stages: [
          ...
          { label: "Запись на площадке", description: "Сотрудник добавляет фотографию, комментарий или замечание и обновляет статус работы." },
      ...
      relatedLinks: [
        {
          label: "Заявки с объекта",
          href: marketingPaths.siteRequests,
          description: "Оформляйте потребности площадки и передавайте заявку ответственному.",
        },
        {
          label: "Программа для прораба",
          href: marketingPaths.foremanSoftware,
  ...
  'pir-project-documentation': createOperationalSeoPage({
    ...
    automationTitle: 'Что автоматизирует',
    automationDescription: 'Версии, замечания и решения о выпуске сохраняются рядом с документами.',
    automations: [
    ...
    visibilityTitle: 'Что видит команда',
    visibilityDescription: 'Проектировщик отвечает на замечания, ПТО проверяет комплект, руководитель видит, что задерживает выпуск.',
    roleViews: [
    ...
    faq: [
      { question: 'Можно ли вести ПД и РД в одной системе?', answer: 'Да. Проектную и рабочую документацию, замечания и комплекты можно связывать с объектом, разделами и статусами.' },
      { question: 'Кто проверяет документацию?', answer: 'Инженерную проверку и нормоконтроль выполняют специалисты. В МОСТ можно сохранить замечание, назначить ответственного и срок ответа, а затем отметить результат проверки.' },
  ...
  'construction-safety': createOperationalSeoPage({
    ...
    audienceDescription: 'Для специалистов по охране труда, прорабов и руководителей, которые отвечают за условия работы на площадке.',
    audiences: [
    ...
    visibilityTitle: 'Что видит команда',
    visibilityDescription: 'Специалист оформляет предписание, прораб организует устранение, руководитель видит открытые нарушения по объектам.',
    roleViews: [
    ...
    faq: [
      ...
      { question: 'МОСТ сам подтверждает безопасность работ?', answer: 'Нет. Система хранит записи, предписания и результаты проверок. Осмотр, допуск к работам и решение об устранении нарушения выполняют ответственные специалисты.' },
    ],
  }),
  ...
  'handover-acceptance': createOperationalSeoPage({
    ...
    faq: [
      ...
      { question: 'Статус в МОСТ заменяет акт приёмки?', answer: 'Нет. Статус помогает отслеживать готовность и замечания. Решение о приёмке и подписание необходимых документов остаются за уполномоченными участниками.' },
    ],
  }),
  'machinery-and-labor': createOperationalSeoPage({
    ...
    automationTitle: 'Что автоматизирует',
    automationDescription: 'Сменный рапорт показывает, какая техника работала, сколько времени и с каким результатом.',
    automations: [
    ...
    faq: [
      ...
      { question: 'Часы работы записываются автоматически?', answer: 'Для учёта нужны заполненные сменные записи или данные подключённого источника. Само добавление машины в МОСТ не включает автоматическое отслеживание её работы.' },
    ],
  }),
  'change-control': createOperationalSeoPage({
    ...
    automationTitle: 'Что автоматизирует',
    automationDescription: 'Вопрос, ответ, документы и согласованное решение доступны в истории изменения.',
    automations: [
    ...
    faq: [
      ...
      { question: 'Сроки и стоимость согласуются автоматически?', answer: 'Нет. Команда готовит оценку, а уполномоченные участники принимают решение. Запись изменения в МОСТ сама по себе не меняет договорные обязательства.' },
    ],
    workflow: {
      ...
      stages: [
        ...
        { label: 'Ответ и предложение', description: 'Ответственный изучает ответ и при необходимости оформляет предложение об изменении.' },
        { label: 'Влияние на проект', description: 'ПТО оценивает влияние на смету, график и бюджет.' },
        { label: 'Согласование', description: 'Уполномоченные участники проверяют предложение и фиксируют решение с документами.' },
