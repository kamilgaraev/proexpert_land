...
describe("normalizeMarketingBlogText", () => {
  it.each([
    ...
    "нормализует только старый бренд и его собственный домен",
    (source, expected) => {
  ...
  it.each([
    [
      "https://example.com/docs/ProHelper",
      "https://example.com/docs/ProHelper",
    ],
    [
      "https://example.com/search?product=ProHelper#ProHelper",
      "https://example.com/search?product=ProHelper#ProHelper",
    ],
    [
    ...
    "сохраняет старый бренд внутри постороннего URL побайтно",
    (source, expected) => {
      expect(normalizeMarketingBlogText(source)).toBe(expected);
    },
  );
  ...
  it.each([
    ...
    "заменяет старый собственный origin перед query, fragment и пунктуацией",
    (source, expected) => {
      expect(normalizeMarketingBlogText(source)).toBe(expected);
    },
  );
  ...
  it.each([
    ...
    "отделяет terminal dot от URL и не принимает чужой hostname за старый origin",
    (source, expected) => {
      expect(normalizeMarketingBlogText(source)).toBe(expected);
    },
  );
  ...
  it.each([
    [
      "https://prohelper.pro@evil.example/path",
      "https://prohelper.pro@evil.example/path",
    ],
    [
      "https://prohelper.pro:secret@evil.example/path",
      "https://prohelper.pro:secret@evil.example/path",
    ],
    [
    ...
    "классифицирует URL по фактическому hostname и безопасно сохраняет malformed token",
    (source, expected) => {
      expect(normalizeMarketingBlogText(source)).toBe(expected);
    },
  );
