...
export const holdingSitesService = {
  // Управление сайтами
  getSites: async (holdingId: number, filters?: any): Promise<{ data: any, status: number, statusText: string }> => {
    const token = getTokenFromStorages();
    const params = new URLSearchParams();
  ...
  createSite: async (holdingId: number, siteData: any): Promise<{ data: any, status: number, statusText: string }> => {
    const token = getTokenFromStorages();
  ...
  // Управление блоками
  getBlocks: async (siteId: number, filters?: any): Promise<{ data: any, status: number, statusText: string }> => {
    const token = getTokenFromStorages();
    const params = new URLSearchParams();
  ...
  getAssets: async (siteId: number, filters?: any): Promise<{ data: any, status: number, statusText: string }> => {
    const token = getTokenFromStorages();
    const params = new URLSearchParams();
  ...
  uploadAsset: async (siteId: number, file: File, usageContext?: string): Promise<{ data: any, status: number, statusText: string }> => {
    const token = getTokenFromStorages();
  ...
  // Публичный API
  getPublicSite: async (domain: string): Promise<{ data: any, status: number, statusText: string }> => {
    const response = await fetch(`${API_URL}/public/sites/${domain}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
  ...
  getPublicSitePreview: async (domain: string, token: string): Promise<{ data: any, status: number, statusText: string }> => {
    const response = await fetch(`${API_URL}/public/sites/${domain}/preview?token=${token}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
