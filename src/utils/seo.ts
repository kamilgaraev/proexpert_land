...
export const getPageSEOData = (page: string): PageSEOData => {
  ...
  const seoData: Record<string, PageSEOData> = {
    ...
    api: {
      title: "API ProHelper - Разработка для строительных систем",
      description: "REST API ProHelper для интеграции учета материалов и управления проектами. Документация, SDK, примеры кода для строительных систем. Техническая поддержка разработчиков.",
      keywords: "API строительство, REST API учет материалов, разработка строительных систем, SDK управление проектами, техническая интеграция стройка, API документация строительство, веб-хуки стройка",
